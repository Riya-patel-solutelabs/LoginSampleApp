...
class SignupFragment : BaseFragment(), DatePickerDialog.OnDateSetListener {
    ...
    private fun onSignup() {
        ...
        if (validation.isValidFname(fname) && validation.isValidLname(lname)
            && validation.isValidEmail(email) && validation.isValidPassword(password) && validation.isValidGender(
                selectedBtn
            ) &&
            validation.isValidDob(dob) && validation.isValidAge(age)
        ) {
            val userDet = UserData(
                null,
                fname,
                lname,
                email,
            ...
            GlobalScope.launch(Dispatchers.Main) {
                ...
                if (user) {
                    showSnackbar(
                        getString(R.string.label_already_registered_error),
                        requireContext()
                    )
                } else {
                    insertData(userDet)
